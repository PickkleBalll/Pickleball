...
public class CoursePackageController : ControllerBase
{
    ...
    // ✅ Cập nhật khóa học
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(string id, [FromBody] UpdateCoursePackageDto updated)
    {
        ...
        // Load lại course kèm coach để trả về đầy đủ thông tin
        var courseWithCoach = await _context.CoursePackages
            .Include(cp => cp.Coach)
            .FirstOrDefaultAsync(cp => cp.PackageId == id);

        return Ok(new { message = "Cập nhật thành công", data = courseWithCoach });
